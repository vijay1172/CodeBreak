...
export function AuthForm({ mode }: { mode: "login" | "signup" }) {
  ...
  return <><SiteHeader/><main id="main-content" tabIndex={-1} className="auth-layout page-width">
    ...
    <form className="auth-form" onSubmit={submit}><h2>{signup ? "Create your account" : "Log in to BrokenRepo"}</h2>
      ...
      <p>{signup ? "Already have an account?" : "New to BrokenRepo?"} <Link className="underlined" href={signup ? "/login" : "/signup"}>{signup ? "Log in" : "Create an account"}</Link></p>
    </form>
  </main><SiteFooter/></>;
}
